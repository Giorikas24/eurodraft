            ...
            <form onSubmit={handleAddGame} className="flex flex-col gap-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  >
                    <option value="">Επέλεξε ομάδα...</option>
                    {EUROLEAGUE_TEAMS.filter(t => t.name !== homeTeam).map(t => (
                      <option key={t.id} value={t.name}>{t.name}</option>
                    ))}
                  </select>
                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="text-xs text-gray-500 mb-1.5 block">
                    Απόδοση {awayTeam || "φιλοξενούμενου"}
                  </label>
                  <input
                    type="number"
                    step="0.01"
              ...
              {points && (
                <div className="bg-[#1a1a1a] rounded-lg px-4 py-3 flex justify-between items-center">
                  ...
                  <div className="flex gap-4">
                    <span className="text-sm font-medium text-white">{homeTeam || "Γηπεδούχος"}: <span className="text-[#ff751f]">+{points.homePoints}</span></span>
                    <span className="text-sm font-medium text-white">{awayTeam || "Φιλοξενούμενος"}: <span className="text-[#ff751f]">+{points.awayPoints}</span></span>
                  </div>
                </div>
              )}
        ...
        <div className="flex flex-col gap-3">
          ...
          {games.length === 0 && (
            <div className="bg-[#111] border border-[#1e1e1e] rounded-xl p-8 text-center text-gray-500 text-sm">
              Δεν υπάρχουν ματς ακόμα. Πάτα "+ Πρόσθεσε ματς" για να ξεκινήσεις.
            </div>
          )}
          {games.map((g) => (
                  ...
                  <button
                    ...
                    className={`flex-1 py-2 rounded-lg text-sm font-medium transition-colors disabled:opacity-50 ${
                      g.result === "home"
                        ...
                        : "bg-[#1a1a1a] border border-[#333] text-white hover:border-[#ff751f]"
                    }`}
                  >
                    {g.homeTeam}
                  </button>
