...
import { collection, addDoc, getDocs, doc, getDoc, updateDoc, orderBy, query, writeBatch } from "firebase/firestore";
...
export default function MatchdayDetailPage() {
  ...
  const fetchMatchday = async () => {
    ...
    if (docSnap.exists()) {
      setMatchday(docSnap.data() as {number: number, deadline: any});
    }
  };
  ...
  const handleSetResult = async (game: Game, result: "home" | "away") => {
    ...
    try {
      ...
      // 2. Βρες όλες τις προβλέψεις για αυτή την αγωνιστική
      ...
      for (const predDoc of predictionsSnap.docs) {
        ...
        const userPick = predData.picks[game.id];
        const isCorrect = userPick === result;
        const pointsEarned = isCorrect
          ? (result === "home" ? game.homePoints : game.awayPoints)
          : -1;

        // 3. Ενημέρωσε τους πόντους του user
        const userRef = doc(db, "users", predData.userId);
        const userSnap = await getDoc(userRef);
        if (userSnap.exists()) {
          const currentPoints = userSnap.data().points || 0;
          batch.update(userRef, { points: currentPoints + pointsEarned });
        }
      }

      await batch.commit();
      fetchGames();
      alert(`Αποτέλεσμα αποθηκεύτηκε! Οι πόντοι ενημερώθηκαν.`);
    } catch (err) {
      console.error(err);
  ...
  return (
    <main className="min-h-screen bg-[#0a0a0a] text-white">
      ...
      <div className="max-w-4xl mx-auto px-10 py-12">
        <h1 className="text-3xl font-medium mb-2">Αγωνιστική #{matchday?.number}</h1>
        <p className="text-gray-500 text-sm mb-8">
          Deadline: {matchday?.deadline ? new Date(matchday.deadline.toDate ? matchday.deadline.toDate() : matchday.deadline).toLocaleString("el-GR") : ""}
        </p>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" value={homeTeam} onChange={(e) => setHomeTeam(e.target.value)}
                  ...
                  placeholder="π.χ. Ολυμπιακός" required />
              </div>
              <div>
                ...
                <input type="text" value={awayTeam} onChange={(e) => setAwayTeam(e.target.value)}
                  className="w-full bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg px-4 py-2.5 text-sm text-white focus:outline-none focus:border-[#ff751f]"
                  placeholder="π.χ. Βιλερμπάν" required />
              </div>
            </div>
        ...
        <div className="flex flex-col gap-3">
          ...
          {games.length === 0 && <div className="text-gray-500 text-sm">Δεν υπάρχουν ματς ακόμα.</div>}
          {games.map((g) => (
              ...
              <div className="flex justify-between items-center mb-3">
                <div>
                  ...
                  <div className="text-xs text-gray-500 mt-1">
                    {new Date(g.date.toDate ? g.date.toDate() : g.date).toLocaleString("el-GR")} · +{g.homePoints} / +{g.awayPoints} πόντοι
                  </div>
                </div>
                <span className={`text-xs px-2 py-1 rounded font-medium ${
                  ...
                  {g.status === "pending" ? "Αναμονή" : g.status === "live" ? "LIVE" : `Τελικό: ${g.result === "home" ? g.homeTeam : g.awayTeam}`}
                </span>
              </div>
              {(g.status !== "finished" || true) && (
                <div className="flex gap-2 mt-2">
                  <span className="text-xs text-gray-500 self-center">Αποτέλεσμα:</span>
                  <button
                    onClick={() => handleSetResult(g, "home")}
                    disabled={grading === g.id}
                    className="text-xs bg-[#1a1a1a] border border-[#333] px-3 py-1.5 rounded-lg hover:border-[#ff751f] hover:text-[#ff751f] disabled:opacity-50"
                  >
                    {g.homeTeam}
                  </button>
                  <button
                    onClick={() => handleSetResult(g, "away")}
                    disabled={grading === g.id}
                    className="text-xs bg-[#1a1a1a] border border-[#333] px-3 py-1.5 rounded-lg hover:border-[#ff751f] hover:text-[#ff751f] disabled:opacity-50"
                  >
                    {g.awayTeam}
                  </button>
                  {grading === g.id && <span className="text-xs text-gray-500 self-center">Βαθμολόγηση...</span>}
                </div>
              )}
            </div>
          ))}
